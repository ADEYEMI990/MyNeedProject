...
//   // Get the decoded userId from the server-side
...
//     // Get userId from the backend by verifying the token
//     const userId = await getUserIdFromToken(token);
//     if (!userId) {
//       toast.error("Unable to retrieve user ID from token");
//       return;
//     }
...
//       (count, itemSizes) =>
//         count + Object.values(itemSizes).reduce((sum, qty) => sum + qty, 0),
//       0
//     );
//   };
...
//           { headers: { Authorization: `Bearer ${token}` } }
//         );
//         await getUserCart(token); // Fetch the updated cart
//       } catch (error) {
